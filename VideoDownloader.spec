...
# vendor 二进制: 缺哪个都会让同事那边直接用不了, 所以打包前先断言存在,
# 别等装到同事电脑上才发现 (gitignore 掉了, 很容易忘记放)
#
# **不打 ffprobe** (96MB, 占原包 24%): 实测 MP4 分离流合并和 MP3 提取都不需要它,
# yt-dlp 只会记一条 warning。本工具不用嵌入缩略图/元数据那些需要探测媒体的功能。
# 若哪天发现某种格式必须要它, 把 "ffmpeg/ffprobe.exe" 加回下面这行即可。
for rel in ("ffmpeg/ffmpeg.exe", "deno/deno.exe"):
    src = _VENDOR / rel
    if not src.is_file():
        raise SystemExit(
            f"打包中止: 缺少 {src}\n"
